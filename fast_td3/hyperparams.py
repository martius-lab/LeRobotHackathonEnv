...
class BaseArgs:
    ...
    """the project name"""
    exp_dir: str = "results"
    """root directory for experiment outputs (checkpoints, logs, etc.)"""
...
def get_args():
    ...
    env_to_args_class = {
        ...
        # LeRobot
        "LeRobot-v0": LeRobotArgs,
        "LeRobotGoalConditioned-v0": LeRobotArgs,
    }
...
@dataclass
class LeRobotArgs(MuJoCoPlaygroundArgs):
    """Reasonable defaults for LeRobot* envs."""

    env_name: str = "LeRobotGoalConditioned-v0"
    num_envs: int = 16
    num_eval_envs: int = 16
    buffer_size: int = 1024 * 10
    batch_size: int = 1024 * 4
    # MetaWorld / MTBench-style value range and discount
    reward_normalization: bool = True
    gamma: float = 0.97
    v_min: float = -10.0
    v_max: float = 10.0
